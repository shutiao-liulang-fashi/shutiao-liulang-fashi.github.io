...
- 优化 YueLiNotes.vue 组件
  ...
  - 重构播放控制区域，添加独立的播放按钮和切换按钮
  - 播放按钮改为非绝对定位，不再悬浮在内容上方
  - 添加视图切换功能，当 showNotes 和 showSheetMusic 都为 true 时显示切换按钮
  - 默认显示音符信息，可切换到五线谱，避免同时占用两个高度
  - 修复五线谱容器使用 v-show 而不是 v-if，确保容器始终存在于 DOM 中
  - 播放按钮和切换按钮样式统一，都显示图标和文字
  - 添加按钮最小宽度，避免文字变化导致的宽度抖动
  - 增加按钮内边距，提升视觉效果
- 优化 YueLiNotesPlayground.vue 组件
  - 引入《夕颜》简谱数据作为默认示例
  - 新增 jianpu_xiyan.ts 模块存储《夕颜》简谱
  - 移除硬编码的示例简谱，改用外部数据源
  - textarea 不自动折行，显示横向和竖向滚动条
  - 转换结果区域也不自动折行，显示滚动条
- 优化 Yueqin.vue 月琴组件
  - 移除硬编码的 ABC 示例数据
  - 清理未使用的 highlightPosition 函数
- 修复 jianpuToAbc.ts 转换规则
  - 修复括号 ( 和 ) 自动添加空格的问题
  - 括号不触发空格，其他特殊符号正常添加空格
  - 确保 (1 2) 正确转换为 (C D)，而不是 ( C D )
- 修复 jianpuToScientific.ts 转换规则
  - 修复括号 ( 和 ) 自动添加空格的问题
  - 括号不触发空格，其他特殊符号正常添加空格
- 修复 scientificToAbc.ts 转换规则
  - 修复括号 ( 和 ) 自动添加空格的问题
  - 括号不触发空格，其他特殊符号正常添加空格
